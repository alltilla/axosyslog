...
static const gchar *
_obj_to_string(FilterXObject *obj)
{
  const gchar *str;
...
_set_subscript(FilterXDict *s, FilterXObject *key, FilterXObject **new_value)
{
  ...
  const gchar *name_str = _obj_to_string(key);
  const gchar *value_str = _obj_to_string(*new_value);
  ...
  StatsClusterLabel label = stats_cluster_label(g_strdup(name_str), g_strdup(value_str));
...
{
  const StatsClusterLabel *lhs = *(const StatsClusterLabel **) a;
  const StatsClusterLabel *rhs = *(const StatsClusterLabel **) b;
...
filterx_object_metrics_labels_get_value_ref(FilterXObject *s, gsize *len)
{
  ...
  if (!self->sorted)
    {
      g_ptr_array_sort(self->labels, _label_cmp);
      self->sorted = TRUE;
    }
