...
_process_dict(FilterXFunctionUnsetEmpties *self, FilterXObject *obj, gint depth)
{
  ...
  FilterXObject *stack_keys[UNSET_EMPTIES_STACK_KEYS];
  FilterXObject **keys_to_unset = (len > UNSET_EMPTIES_STACK_KEYS) ? g_new(FilterXObject *, len) : stack_keys;
