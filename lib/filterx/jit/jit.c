...
/* set up the per-block module's debug flags, DIBuilder and compile unit */
static void
...
filterx_jit_ir_add_new_block(FilterXJIT *self, const gchar *block_name)
{
  ...
  /* one module per block; the cached jit->ffi handles are module-scoped, so re-declare them here */
  g_assert(!self->mod);
...
/* Render the block's function (one instruction per line) into a memfd shared by all blocks, so a
 * debugger can map the JIT'd code back to the IR. Text and line numbering accumulate across blocks
 * and are flushed to the memfd in finalize(). */
static gboolean
...
/* Capture the finished block and deduplicate on its generated code: identical blocks (which differ
 * only in their function name and source-location debug info) share one compiled __fx_block_<hash>
 * function. Each instance keeps its own ptr_table of pointers, supplied to the shared function at
 * call time. */
static void
...
/* Parse a private copy of a shared bitcode buffer into ctx (each worker owns its modules). */
...
/* Optimize (O3) and codegen a linked bucket module to a native object buffer; NULL on failure. */
...
_parallel_compile_worker(gpointer data, gpointer user_data)
{
  ...
  guint bucket = GPOINTER_TO_UINT(data) - 1; /* +1 on push: GThreadPool rejects a NULL task pointer */
...
_finalize_parallel(FilterXJIT *self, GError **error)
{
  ...
  guint n_buckets = (guint) g_get_num_processors(); /* one compile unit per core */
  if (n_buckets > nblocks)
    n_buckets = nblocks;

  msg_trace("FilterX JIT parallel finalize", evt_tag_int("blocks", nblocks),
            evt_tag_int("buckets", n_buckets), evt_tag_int("threads", threads));
  ...
  for (guint b = 0; b < n_buckets; b++)
    g_thread_pool_push(pool, GUINT_TO_POINTER(b + 1), NULL); /* +1: NULL task is rejected by the pool */
  g_thread_pool_free(pool, FALSE, TRUE); /* wait for all workers */
...
#if FILTERX_JIT_DEBUG_INFO_LLVM_IR_SUPPORTED
  /* flush the IR text referenced by the blocks' debug info */
...
_setup_debug_info(FilterXJIT *self, LLVMOrcLLJITBuilderRef jit_builder)
{
  ...
  /* debug info is set up per block; here we only install the gdb JIT event listener */
}
...
filterx_jit_free(FilterXJIT *self)
{
  ...
  self->ctx = NULL;
  /* self->mod is NULL between blocks (each block's module is captured and disposed in finish());
   * it is non-NULL only while a block is being built. */
